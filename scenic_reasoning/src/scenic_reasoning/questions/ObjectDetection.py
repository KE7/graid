...
class IsObjectCentered(Question):
    def __init__(self) -> None:
        super().__init__(
            question="Is <object_1> centered in the image, or is it off to the left or right?",
            variables=["object_1"],
            predicates=[
    ...
    def apply(
        ...
        single_detections = [class_name for class_name, count in detection_counts.items() if count == 1]
            ...
            if type(class_name) == torch.Tensor: # shape == (# of boxes,)
                ...
                for class_name in class_name:
                    if class_name in single_detections:
                        object_positions.append((class_name, detection.as_xyxy()[0][0], detection.as_xyxy()[0][2]))
            else:
                if class_name in single_detections:
                    object_positions.append((class_name, detection.as_xyxy()[0][0], detection.as_xyxy()[0][2]))
        ...
        for class_name, x_min, x_max in object_positions:
            question = f"Is {class_name} centered in the image, or is it off to the left or right?"
