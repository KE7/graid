...
class TestImageLoaderTransformFix:
    ...
    def test_transform_receives_both_image_and_labels(self):
        ...
        with patch('graid.data.ImageLoader.ImageDataset.__init__', 
                   return_value=None):
            ...
            with patch('cv2.imread', return_value=mock_image):
                with patch('cv2.cvtColor', return_value=mock_image):
                    ...
                    try:
                        ...
                        assert len(call_args) == 2, "Transform should receive exactly 2 arguments (image, labels)"
                        
                        # Verify the arguments are image and labels
                        image_arg, labels_arg = call_args
                        assert isinstance(image_arg, np.ndarray), "First argument should be image (numpy array)"
                        assert isinstance(labels_arg, list), "Second argument should be labels (list)"
                        
                    except Exception as e:
                        # If there's still an error, it should not be about transform signature
                        assert "takes 2 positional arguments but 3 were given" not in str(e)
                ...
                if 'transforms' in config:
                    ...
                    try:
                        if 'train' in transforms:
                            ...
                            assert len(result) == 2, f"Train transform for {backend}.{model_name} should return (image, labels)"
                        
                        if 'val' in transforms:
                            result = transforms['val'](mock_image, mock_labels)
                            assert len(result) == 2, f"Val transform for {backend}.{model_name} should return (image, labels)"
                            
                    except Exception as e:
                        pytest.fail(f"Transform for {backend}.{model_name} failed: {str(e)}") 