...
class ErrorHandler:
    ...
    def handle_validation_error(error: ValidationError):
        ...
        typer.secho(f"❌ Validation Error: {error.message}", fg=typer.colors.RED, bold=True)
    ...
    def handle_configuration_error(error: ConfigurationError):
        ...
        typer.secho(f"❌ Configuration Error: {error.message}", fg=typer.colors.RED, bold=True)
    ...
    def handle_processing_error(error: ProcessingError):
        ...
        typer.secho(f"❌ Processing Error: {error.message}", fg=typer.colors.RED, bold=True)
        typer.echo("💡 This usually indicates an issue with the dataset or model configuration.")
    ...
    def handle_upload_error(error: UploadError):
        ...
        typer.echo("💡 Check your HuggingFace Hub authentication and network connection.")
    ...
    def handle_unexpected_error(error: Exception):
        ...
        typer.echo("💡 This is likely a bug. Please report it with the traceback below.")
        typer.echo()
