...
def test_threshold_functionality():
    ...
    mmdet_model = MMdetection_obj(
        config_file=str(MMDETECTION_PATH /
    ...
    if isinstance(detections_default, list) and len(detections_default) > 0 and isinstance(detections_default[0], list):
    ...
    # Handle batch format - get first batch
    if isinstance(detections_low, list) and len(detections_low) > 0 and isinstance(detections_low[0], list):
    ...
    # Handle batch format - get first batch
    if isinstance(detections_high, list) and len(detections_high) > 0 and isinstance(detections_high[0], list):
    ...
    print("✅ Ultralytics: Uses built-in conf parameter with proper image transformations")
