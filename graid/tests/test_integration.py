    ...
    def _create_sample_detections(self):
        ...
        return [
            ObjectDetectionResultI(
                ...
                bbox=[100, 200, 300, 350], image_hw=(480, 640)
            ),
            ObjectDetectionResultI(
                score=0.92, cls=0, label="person", 
                bbox=[320, 150, 380, 340], image_hw=(480, 640)
            ),
            ObjectDetectionResultI(
                score=0.65, cls=21, label="elephant",  # Should be filtered
                bbox=[200, 180, 400, 380], image_hw=(480, 640)
            )
        ]
...
class TestHumanSupervisedIntegration(unittest.TestCase):
    ...
    def test_human_supervised_filter_integration(self):
        ...
        detection = ObjectDetectionResultI(
            score=0.8, cls=2, label="car",
            bbox=[100, 100, 200, 200], image_hw=(224, 224)
        )
        
        # Test validation
        result = filter_obj.validate_detection(
            detection,
            Image.open(self.sample_image_path),
            "How many cars?",
            "1"
        )
...
class TestPerformanceAndScaling(unittest.TestCase):
    ...
    def test_memory_efficiency(self):
        ...
        detections = [
            ObjectDetectionResultI(
                score=0.7, cls=1, label="test",
                bbox=[10, 10, 50, 50], image_hw=(100, 100)
            )
            for _ in range(10)
        ]
